...
public final class ValueRange implements Serializable {
    ...
    public int checkValidIntValue(long j, OooOO0 oooOO02) {
        if (isValidIntValue(j)) {
            return (int) j;
        }
        throw new DateTimeException("Invalid int value for " + oooOO02 + ": " + j);
    }

    public long checkValidValue(long j, OooOO0 oooOO02) {
        if (isValidValue(j)) {
            return j;
        }
        if (oooOO02 == null) {
        ...
        throw new DateTimeException("Invalid value for " + oooOO02 + " (valid values " + this + "): " + j);
    }
