...
public final class Contrast {
    ...
    public static double darker(double d, double d2) {
        if (d >= IDataEditor.DEFAULT_NUMBER_VALUE && d <= 100.0d) {
            double dYFromLstar = ColorUtils.yFromLstar(d);
            double d3 = ((dYFromLstar + 5.0d) / d2) - 5.0d;
            if (d3 >= IDataEditor.DEFAULT_NUMBER_VALUE && d3 <= 100.0d) {
                ...
                if (dLstarFromY >= IDataEditor.DEFAULT_NUMBER_VALUE && dLstarFromY <= 100.0d) {
                    return dLstarFromY;
                }
            }
        }
    ...
    public static double darkerUnsafe(double d, double d2) {
        return Math.max(IDataEditor.DEFAULT_NUMBER_VALUE, darker(d, d2));
    }
    ...
    public static double lighterUnsafe(double d, double d2) {
        ...
        if (dLighter < IDataEditor.DEFAULT_NUMBER_VALUE) {
            return 100.0d;
        }
        return dLighter;
    }
