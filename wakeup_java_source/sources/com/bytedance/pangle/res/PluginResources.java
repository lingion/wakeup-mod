...
public class PluginResources extends Resources {
    ...
    public static AssetManager appendHostRes(Resources resources) {
        String strH = yv.h(Zeus.getAppApplication());
        String strBj = yv.bj(Zeus.getAppApplication());
        List<String> listBj = qo.bj();
        h hVar = new h();
        AssetManager assets = resources.getAssets();
        AssetManager assets2 = Zeus.getAppApplication().getAssets();
        HashSet hashSet = new HashSet(qo.h(assets));
        List<String> listH = qo.h(assets2);
        for (String str : listBj) {
            if (!hashSet.contains(str)) {
                assets = hVar.h(assets, str, true);
            }
        }
        for (String str2 : listH) {
            if (!isOtherPlugin(str2, strH, strBj) && !hashSet.contains(str2) && !listBj.contains(str2)) {
                assets = hVar.h(assets, str2, false);
            }
        }
        ZeusLogger.i(ZeusLogger.TAG_RESOURCES, "pluginAssets = " + qo.bj(assets));
        return assets;
    }

    private Resources.NotFoundException handleException(Resources.NotFoundException notFoundException) {
        return new Resources.NotFoundException(("Resources#Assets: " + qo.bj(getAssets())) + "," + notFoundException.getMessage());
    }
    ...
    private static boolean isOtherPlugin(String str, String str2, String str3) {
        ...
        ZeusLogger.d(ZeusLogger.TAG_RESOURCES, str + ServerSentEventKt.SPACE + packageResourcePath + ServerSentEventKt.SPACE + str2 + ServerSentEventKt.SPACE + str3);
