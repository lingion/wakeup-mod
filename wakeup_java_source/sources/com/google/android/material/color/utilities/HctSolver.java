...
public class HctSolver {
    ...
    static double[][] bisectToSegment(double d, double d2) {
        ...
        for (int i = 0; i < 12; i++) {
            ...
            if (dArrNthVertex[0] >= IDataEditor.DEFAULT_NUMBER_VALUE) {
                ...
                if (!z) {
                    ...
                    d4 = d3;
                    z = true;
                } else if (z2 || areInCyclicOrder(d3, dHueOf, d4)) {
                    if (areInCyclicOrder(d3, d2, dHueOf)) {
                        dArr2 = dArrNthVertex;
                        d4 = dHueOf;
                    } else {
                        dArr = dArrNthVertex;
    ...
    static double chromaticAdaptation(double d) {
        ...
        return ((MathUtils.signum(d) * 400.0d) * dPow) / (dPow + 27.13d);
    }
    ...
    static int findResultByJ(double d, double d2, double d3) {
        ...
        while (i < 5) {
            ...
            double d6 = dCos;
            double dPow2 = Math.pow(((d2 == IDataEditor.DEFAULT_NUMBER_VALUE || dSqrt == IDataEditor.DEFAULT_NUMBER_VALUE) ? 0.0d : d2 / Math.sqrt(d5)) * dPow, 1.1111111111111112d);
            double aw = (viewingConditions.getAw() * Math.pow(d5, (d4 / viewingConditions.getC()) / viewingConditions.getZ())) / viewingConditions.getNbb();
            double d7 = (((0.305d + aw) * 23.0d) * dPow2) / (((23.0d * d6) + ((dPow2 * 11.0d) * dCos2)) + ((dPow2 * 108.0d) * dSin));
            double d8 = d7 * dCos2;
            double d9 = d7 * dSin;
            double d10 = aw * 460.0d;
            double[] dArrMatrixMultiply = MathUtils.matrixMultiply(new double[]{inverseChromaticAdaptation((((451.0d * d8) + d10) + (288.0d * d9)) / 1403.0d), inverseChromaticAdaptation(((d10 - (891.0d * d8)) - (261.0d * d9)) / 1403.0d), inverseChromaticAdaptation(((d10 - (d8 * 220.0d)) - (d9 * 6300.0d)) / 1403.0d)}, LINRGB_FROM_SCALED_DISCOUNT);
            double d11 = dArrMatrixMultiply[0];
            if (d11 < IDataEditor.DEFAULT_NUMBER_VALUE) {
                break;
            }
            double d12 = dArrMatrixMultiply[1];
            if (d12 < IDataEditor.DEFAULT_NUMBER_VALUE) {
                break;
            }
            double d13 = dArrMatrixMultiply[2];
            if (d13 < IDataEditor.DEFAULT_NUMBER_VALUE) {
                break;
            }
            double[] dArr = Y_FROM_LINRGB;
            double d14 = (dArr[0] * d11) + (dArr[1] * d12) + (dArr[2] * d13);
            if (d14 <= IDataEditor.DEFAULT_NUMBER_VALUE) {
                return 0;
            }
            if (i != 4) {
                double d15 = d14 - d3;
                if (Math.abs(d15) >= 0.002d) {
                    dSqrt -= (d15 * dSqrt) / (d14 * 2.0d);
                    i++;
                    dCos = d6;
                    d4 = 1.0d;
                }
    ...
    static double inverseChromaticAdaptation(double d) {
        ...
        return MathUtils.signum(d) * Math.pow(Math.max(IDataEditor.DEFAULT_NUMBER_VALUE, (27.13d * dAbs) / (400.0d - dAbs)), 2.380952380952381d);
    }

    static boolean isBounded(double d) {
        return IDataEditor.DEFAULT_NUMBER_VALUE <= d && d <= 100.0d;
    }
