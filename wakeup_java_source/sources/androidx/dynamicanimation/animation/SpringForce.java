...
public final class SpringForce implements Force {
    ...
    private void init() {
        ...
        } else if (d >= IDataEditor.DEFAULT_NUMBER_VALUE && d < 1.0d) {
    ...
    public SpringForce setDampingRatio(@FloatRange(from = IDataEditor.DEFAULT_NUMBER_VALUE) float f) {
        if (f < 0.0f) {
    ...
    public SpringForce setStiffness(@FloatRange(from = IDataEditor.DEFAULT_NUMBER_VALUE, fromInclusive = false) float f) {
        if (f <= 0.0f) {
    ...
    DynamicAnimation.MassState updateValues(double d, double d2, long j) {
        ...
        if (d5 > 1.0d) {
            ...
            double dPow4 = Math.pow(2.718281828459045d, (-d5) * d17 * d3) * ((Math.cos(this.mDampedFreq * d3) * d4) + (Math.sin(this.mDampedFreq * d3) * d18));
