...
public final class Cam16 {
    ...
    private final double[] tempArray = {IDataEditor.DEFAULT_NUMBER_VALUE, IDataEditor.DEFAULT_NUMBER_VALUE, IDataEditor.DEFAULT_NUMBER_VALUE};
    ...
    private static Cam16 fromJchInViewingConditions(double d, double d2, double d3, ViewingConditions viewingConditions) {
        ...
        return new Cam16(d3, d2, d, c, flRoot, dSqrt, d5, Math.cos(radians) * dLog1p, Math.sin(radians) * dLog1p);
    }
    ...
    public static Cam16 fromUcsInViewingConditions(double d, double d2, double d3, ViewingConditions viewingConditions) {
        ...
        if (dAtan2 < IDataEditor.DEFAULT_NUMBER_VALUE) {
            dAtan2 += 360.0d;
        }
    ...
    static Cam16 fromXyzInViewingConditions(double d, double d2, double d3, ViewingConditions viewingConditions) {
        ...
        if (degrees < IDataEditor.DEFAULT_NUMBER_VALUE) {
            degrees += 360.0d;
        ...
        return new Cam16(d15, dSqrt, dPow4, c, flRoot, Math.sqrt((dPow5 * viewingConditions.getC()) / (viewingConditions.getAw() + 4.0d)) * 50.0d, (1.7000000000000002d * dPow4) / ((0.007d * dPow4) + 1.0d), dLog1p * Math.cos(radians), dLog1p * Math.sin(radians));
    }
    ...
    double[] xyzInViewingConditions(ViewingConditions viewingConditions, double[] dArr) {
        double dPow = Math.pow(((getChroma() == IDataEditor.DEFAULT_NUMBER_VALUE || getJ() == IDataEditor.DEFAULT_NUMBER_VALUE) ? 0.0d : getChroma() / Math.sqrt(getJ() / 100.0d)) / Math.pow(1.64d - Math.pow(0.29d, viewingConditions.getN()), 0.73d), 1.1111111111111112d);
        ...
        double dSignum = Math.signum(d5) * (100.0d / viewingConditions.getFl()) * Math.pow(Math.max(IDataEditor.DEFAULT_NUMBER_VALUE, (Math.abs(d5) * 27.13d) / (400.0d - Math.abs(d5))), 2.380952380952381d);
        double dSignum2 = Math.signum(d6) * (100.0d / viewingConditions.getFl()) * Math.pow(Math.max(IDataEditor.DEFAULT_NUMBER_VALUE, (Math.abs(d6) * 27.13d) / (400.0d - Math.abs(d6))), 2.380952380952381d);
        double dSignum3 = Math.signum(d7) * (100.0d / viewingConditions.getFl()) * Math.pow(Math.max(IDataEditor.DEFAULT_NUMBER_VALUE, (Math.abs(d7) * 27.13d) / (400.0d - Math.abs(d7))), 2.380952380952381d);
