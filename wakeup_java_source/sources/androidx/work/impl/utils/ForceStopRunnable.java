...
public class ForceStopRunnable implements Runnable {
    ...
    public boolean cleanUp() {
        boolean zReconcileJobs = Build.VERSION.SDK_INT >= 23 ? SystemJobScheduler.reconcileJobs(this.mContext, this.mWorkManager.getWorkDatabase()) : false;
    ...
    public boolean isForceStopped() {
        try {
            ...
            PendingIntent pendingIntent = getPendingIntent(this.mContext, i >= 31 ? 570425344 : C.ENCODING_PCM_A_LAW);
            if (i >= 30) {
                if (pendingIntent != null) {
                    pendingIntent.cancel();
                }
                List historicalProcessExitReasons = ((ActivityManager) this.mContext.getSystemService(TTDownloadField.TT_ACTIVITY)).getHistoricalProcessExitReasons(null, 0, 0);
                if (historicalProcessExitReasons != null && !historicalProcessExitReasons.isEmpty()) {
                    long lastForceStopEventMillis = this.mPreferenceUtils.getLastForceStopEventMillis();
                    for (int i2 = 0; i2 < historicalProcessExitReasons.size(); i2++) {
                        ApplicationExitInfo applicationExitInfoOooO00o = OooOO0.OooO00o(historicalProcessExitReasons.get(i2));
                        if (applicationExitInfoOooO00o.getReason() == 10 && applicationExitInfoOooO00o.getTimestamp() >= lastForceStopEventMillis) {
                            return true;
                        }
                    }
                }
