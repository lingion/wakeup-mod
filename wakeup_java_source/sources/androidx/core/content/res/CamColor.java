...
public class CamColor {
    ...
    private static CamColor findCamByJ(@FloatRange(from = IDataEditor.DEFAULT_NUMBER_VALUE, to = 360.0d) float f, @FloatRange(from = IDataEditor.DEFAULT_NUMBER_VALUE, to = Double.POSITIVE_INFINITY, toInclusive = false) float f2, @FloatRange(from = IDataEditor.DEFAULT_NUMBER_VALUE, to = 100.0d) float f3) {
        float f4 = 100.0f;
        float f5 = 1000.0f;
    ...
    static void fromColorInViewingConditions(@ColorInt int i, ViewingConditions viewingConditions, @Size(7) float[] fArr, @Size(MediaDescriptionCompat.BT_FOLDER_TYPE_ARTISTS) float[] fArr2) {
    ...
    private static CamColor fromJch(@FloatRange(from = IDataEditor.DEFAULT_NUMBER_VALUE, to = 100.0d) float f, @FloatRange(from = IDataEditor.DEFAULT_NUMBER_VALUE, to = Double.POSITIVE_INFINITY, toInclusive = false) float f2, @FloatRange(from = IDataEditor.DEFAULT_NUMBER_VALUE, to = 360.0d) float f3) {
        return fromJchInFrame(f, f2, f3, ViewingConditions.DEFAULT);
    }

    private static CamColor fromJchInFrame(@FloatRange(from = IDataEditor.DEFAULT_NUMBER_VALUE, to = 100.0d) float f, @FloatRange(from = IDataEditor.DEFAULT_NUMBER_VALUE, to = Double.POSITIVE_INFINITY, toInclusive = false) float f2, @FloatRange(from = IDataEditor.DEFAULT_NUMBER_VALUE, to = 360.0d) float f3, ViewingConditions viewingConditions) {
    ...
    public static void getM3HCTfromColor(@ColorInt int i, @Size(MediaDescriptionCompat.BT_FOLDER_TYPE_ARTISTS) float[] fArr) {
        fromColorInViewingConditions(i, ViewingConditions.DEFAULT, null, fArr);
        fArr[2] = CamUtils.lStarFromInt(i);
    }

    public static int toColor(@FloatRange(from = IDataEditor.DEFAULT_NUMBER_VALUE, to = 360.0d) float f, @FloatRange(from = IDataEditor.DEFAULT_NUMBER_VALUE, to = Double.POSITIVE_INFINITY, toInclusive = false) float f2, @FloatRange(from = IDataEditor.DEFAULT_NUMBER_VALUE, to = 100.0d) float f3) {
    ...
    @FloatRange(from = IDataEditor.DEFAULT_NUMBER_VALUE, to = Double.POSITIVE_INFINITY, toInclusive = false)
    float getChroma() {
        return this.mChroma;
    }

    @FloatRange(from = IDataEditor.DEFAULT_NUMBER_VALUE, to = 360.0d, toInclusive = false)
    float getHue() {
        return this.mHue;
    }

    @FloatRange(from = IDataEditor.DEFAULT_NUMBER_VALUE, to = 100.0d)
    float getJ() {
        return this.mJ;
    }
    ...
    int viewed(ViewingConditions viewingConditions) {
        float fPow = (float) Math.pow(((((double) getChroma()) == IDataEditor.DEFAULT_NUMBER_VALUE || ((double) getJ()) == IDataEditor.DEFAULT_NUMBER_VALUE) ? 0.0f : getChroma() / ((float) Math.sqrt(getJ() / 100.0d))) / Math.pow(1.64d - Math.pow(0.29d, viewingConditions.getN()), 0.73d), 1.1111111111111112d);
        ...
        float fSignum = Math.signum(f5) * (100.0f / viewingConditions.getFl()) * ((float) Math.pow((float) Math.max(IDataEditor.DEFAULT_NUMBER_VALUE, (Math.abs(f5) * 27.13d) / (400.0d - Math.abs(f5))), 2.380952380952381d));
        float fSignum2 = Math.signum(f6) * (100.0f / viewingConditions.getFl()) * ((float) Math.pow((float) Math.max(IDataEditor.DEFAULT_NUMBER_VALUE, (Math.abs(f6) * 27.13d) / (400.0d - Math.abs(f6))), 2.380952380952381d));
        float fSignum3 = Math.signum(((f4 - (f2 * 220.0f)) - (f3 * 6300.0f)) / 1403.0f) * (100.0f / viewingConditions.getFl()) * ((float) Math.pow((float) Math.max(IDataEditor.DEFAULT_NUMBER_VALUE, (Math.abs(r8) * 27.13d) / (400.0d - Math.abs(r8))), 2.380952380952381d));
    ...
    @ColorInt
    static int toColor(@FloatRange(from = IDataEditor.DEFAULT_NUMBER_VALUE, to = 360.0d) float f, @FloatRange(from = IDataEditor.DEFAULT_NUMBER_VALUE, to = Double.POSITIVE_INFINITY, toInclusive = false) float f2, @FloatRange(from = IDataEditor.DEFAULT_NUMBER_VALUE, to = 100.0d) float f3, ViewingConditions viewingConditions) {
        if (f2 < 1.0d || Math.round(f3) <= IDataEditor.DEFAULT_NUMBER_VALUE || Math.round(f3) >= 100.0d) {
