...
public class VTodo extends ICalComponent {
    ...
    protected void validate(List<ICalComponent> list, ICalVersion iCalVersion, List<ValidationWarning> list2) {
        if (iCalVersion != ICalVersion.V1_0) {
            ...
            checkOptionalCardinality(list2, Classification.class, Completed.class, Created.class, Description.class, DateStart.class, Geo.class, LastModified.class, Location.class, Organizer.class, PercentComplete.class, Priority.class, RecurrenceId.class, Sequence.class, Status.class, Summary.class, Url.class);
        }
        checkOptionalCardinality(list2, Color.class);
        checkStatus(list2, AnonymousClass1.$SwitchMap$biweekly$ICalVersion[iCalVersion.ordinal()] != 1 ? new Status[]{Status.needsAction(), Status.completed(), Status.inProgress(), Status.cancelled()} : new Status[]{Status.needsAction(), Status.completed(), Status.accepted(), Status.declined(), Status.delegated(), Status.sent()});
        ICalDate iCalDate = (ICalDate) ValuedProperty.getValue(getDateStart());
        ICalDate iCalDate2 = (ICalDate) ValuedProperty.getValue(getDateDue());
        ...
        if (iCalDate2 != null && duration != null) {
            list2.add(new ValidationWarning(24, new Object[0]));
        }
        if (iCalDate == null && duration != null) {
    ...
    public DateDue setDateDue(Date date, boolean z) {
