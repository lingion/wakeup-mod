...
final class LinearIndeterminateDisjointAnimatorDelegate extends IndeterminateAnimatorDelegate<ObjectAnimator> {
    ...
    private static final int[] DURATION_TO_MOVE_SEGMENT_ENDS = {MediaPlayer.MEDIA_PLAYER_OPTION_SET_LIVE_ABR_CHECK_ENHANCE, MediaPlayer.MEDIA_PLAYER_OPTION_LATENCYTIME, MediaPlayer.MEDIA_PLAYER_OPTION_SUB_FIRST_LOAD_TIME, 750};
    private static final int[] DELAY_TO_MOVE_SEGMENT_ENDS = {1267, 1000, MediaPlayer.MEDIA_PLAYER_OPTION_ENABLE_VIDEO_FRAME_META_CALLBACK, 0};
    ...
    private void maybeUpdateSegmentColors() {
        if (this.dirtyColors) {
            Iterator<DrawingDelegate.ActiveIndicator> it2 = this.activeIndicators.iterator();
            while (it2.hasNext()) {
                it2.next().color = this.baseSpec.indicatorColors[this.indicatorColorIndex];
            }
    ...
    void resetPropertiesForNewStart() {
        ...
        Iterator<DrawingDelegate.ActiveIndicator> it2 = this.activeIndicators.iterator();
        while (it2.hasNext()) {
            it2.next().color = this.baseSpec.indicatorColors[0];
        }
    }
